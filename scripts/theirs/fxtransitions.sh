...
if convert -quiet "$infile1" +repage "$tmpA"; then
  : ' Do Nothing '
else
  errMsg "--- FILE $infile DOES NOT EXIST OR IS NOT AN ORDINARY FILE, NOT READABLE OR HAS ZERO SIZE ---"
fi

if convert -quiet "$infile2" +repage "$tmpC"; then
  : ' Do Nothing '
else
  errMsg "--- FILE $infile DOES NOT EXIST OR IS NOT AN ORDINARY FILE, NOT READABLE OR HAS ZERO SIZE ---"
fi
...
if [ "$reverse" = "yes" -a "$effect" = "recursion" ]; then
  echo ""
  ...
  convert \( $tmp1 -coalesce \) \( $tmp0 -coalesce -delete 0,-1 \) \
    -quiet -layers Optimize $tmp0
elif [ "$reverse" = "yes" -a "$effect" != "recursion" ]; then
  echo ""
